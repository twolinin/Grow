...
{	
	...
	std::cout << result.totalSeedNum << "\n";
	
	
	
	for(int i=0 ; i < seedVec.size() ; i++ )
	{
		std::cout << seedVec[i].seedStr << "\n";

		BWTInterval interval = BWTAlgorithms::findInterval(m_params.indices,seedVec[i].seedStr) ;
		BWTInterval rinterval = BWTAlgorithms::findInterval(m_params.indices,reverseComplement(seedVec[i].seedStr)) ;
		
		std::cout << interval << "\n";
		std::cout << rinterval << "\n";
	}
	/*
	BWTInterval interval = BWTAlgorithms::findInterval(m_params.indices,seedVec[0].seedStr) ;
	BWTInterval rinterval = BWTAlgorithms::findInterval(m_params.indices,reverseComplement(seedVec[0].seedStr)) ;
		
	std::cout << interval.lower << "     lower\n";
	char b;
    int64_t idx = interval.lower;
	int64_t new_idx;

    while(1)
    {
        b = m_params.indices.pBWT->getChar(idx);
        new_idx = m_params.indices.pBWT->getPC(b) + m_params.indices.pBWT->getOcc(b, idx - 1);
        //new_idx = m_params.indices.pBWT->getPC(b) + m_params.indices.pRBWT->getOcc(b, idx - 1);
	...
	std::cout << BWTAlgorithms::extractString(m_params.indices.pBWT,new_idx) << "\n";
