...
// Main
//
int FrequencyMain(int argc, char** argv)
{
    FrequencyOptions(argc, argv);

    // Set the Frequency parameters
    FrequencyParameters ecParams;
    BWT *pBWT, *pRBWT;
...
/*
	...
	*/		
    ...
    FrequencyPostProcess postProcessor(ecParams/*, pGraph*/);
    
    if(opt::numThreads <= 1)
    {
        // Serial mode
        FrequencyProcess processor(ecParams);
        
        SequenceProcessFramework::processSequencesSerial
		<SequenceWorkItem, FrequencyResult, FrequencyProcess, FrequencyPostProcess>
		(opt::readsFile, &processor, &postProcessor);
    }
    else
    {
        // Parallel mode
        std::vector<FrequencyProcess*> processorVector;
        for(int i = 0; i < opt::numThreads; ++i)
        {
            FrequencyProcess* pProcessor = new FrequencyProcess(ecParams);
...
/*
    if(opt::isFirst)
    {
        ...
        SGFastaVisitor av("Frequency_first_Overlap.fa");
        pGraph->visit(av);
    }
    else if(opt::isSecond)
    {
        ...
        SGFastaVisitor av("Frequency_second_NonOverlap.fa");
        pGraph->visit(av);
    }
    else if(opt::isThird)
    {
        ...
        SGFastaVisitor av("Frequency_third_NonOverlapByTwoPB.fa");
...
//
void FrequencyOptions(int argc, char** argv)
{
